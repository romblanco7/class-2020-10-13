...
tibble(replicate= 1:5) %>%
  ...
  mutate(num_red = map_dbl(samp, ~ sum(.$color == "red"))) %>%
  mutate(num_blue = map_dbl(samp, ~ sum(.$color == "blue"))) %>%
  mutate(red_wins = num_red > num_blue) %>%
...
```{r}
tibble(prob = rbinom (n = 1, size = 25, prob = .01))
```
